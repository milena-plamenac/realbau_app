...
namespace realbau_app.api.Controllers
{
    ...
    {
        [HttpGet("{pop}/{hbfinished}/{tfinished}/{ffinished}/{mfinished}/{afinished}/{vfinished}")]
        public async Task<ActionResult<IEnumerable<AddressDB>>> Addresses(string pop, bool hbfinished, bool tfinished, bool ffinished, bool mfinished, bool afinished, bool vfinished)
        {
            try
            {
                ...
                using (var con = new SqlConnection("Server=173.249.2.130,1433\\SQLEXPRESS;Database=realbau_db;User Id=realbau;Password=p4x/yRNf;TrustServerCertificate=True"))
                {
                    ...
                    cmd.Parameters.AddWithValue("@pop", (pop == "*") ? DBNull.Value : pop);
                    cmd.Parameters.AddWithValue("@hbfinished", (hbfinished) ? 1: 0);
                    cmd.Parameters.AddWithValue("@tfinished", (tfinished) ? 1 : 0);
                    cmd.Parameters.AddWithValue("@ffinished", (ffinished) ? 1 : 0);
                    cmd.Parameters.AddWithValue("@mfinished", (mfinished) ? 1 : 0);
                    cmd.Parameters.AddWithValue("@afinished", (afinished) ? 1 : 0);
                    cmd.Parameters.AddWithValue("@vfinished", (vfinished) ? 1 : 0);
