...
namespace realbau_app.api.Controllers
{
    ...
    public class MontazeTermController : ControllerBase
    {
        ...
        public MontazeTermController(IMontazeTermRepository montazeTermRepository)
        {
            this.montazeTermRepository = montazeTermRepository;
        }

        [HttpGet("{city}/{pop}/{year}/{month}/{date}")]
        public async Task<IEnumerable<MontazeTermDB>> Get(string city, string pop, int year, int month, int date)
        {
            return await this.montazeTermRepository.MontazeTermsForDate(city, pop, year, month, date);
           
        }


        [HttpPost]
        public async Task<MontazeTermDB> Post([FromBody] MontazeTermDB term)
        {
            return await this.montazeTermRepository.Insert(term);
        }

        [HttpPut("{id}")]
        public async Task<int> Put(int id, [FromBody] MontazeTermDB term)
        {
            return await this.montazeTermRepository.Update(id, term);
        }
        ...
        {
            return await this.montazeTermRepository.Delete(id);
        }
