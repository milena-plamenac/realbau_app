...
{
    public class MontazeRepository: IMontazeRepository
    {
        public async Task<MontazeDB> Get(int address_id)
        {
            try
            {
        ...
        public async Task<int> Insert(int address_id, MontazeDB montaze)
        {
            try
            {
        ...
        public async Task<int> Update(int address_id, MontazeDB montaze)
        {
            // TODO: Not forget to updated busy glag to old terms if reserved
            try
            {
                MontazeDB montazeDB = await this.Get(address_id);
                using (SqlConnection connection = new SqlConnection("Server=173.249.2.130,1433\\SQLEXPRESS;Database=realbau_db;User Id=realbau;Password=p4x/yRNf;TrustServerCertificate=True"))
                {
